...
function scr_ui_display_weapons(rl, termi_tartaros, equiped_weapon) {
    ...
    if (rl == 1) then fix_left = 0;
    if (rl == 2) then fix_right = 0;

    // ** Ranged Weapons ** //this could be a switch?
    ...
    if (termi_tartaros >= 1) {
        if (rl == 1) and(ui_arm[1] == false) and (fix_left == 0) then fix_left = 1;
        if (rl == 2) and(ui_arm[2] == false) and (fix_right == 0) then fix_right = 1;
    }
    ...
    if (display_type == "lightning_claw") {
        if (termi_tartaros == 0) and(rl == 1) {
            ui_xmod[rl] += 11;
        }
        if (termi_tartaros == 0) and(rl == 2) {
            ui_xmod[rl] -= 8;
        }
        if (termi_tartaros > 0) {
            ui_arm[rl] = false;
