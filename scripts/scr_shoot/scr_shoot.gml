function scr_shoot(weapon_number, target_object, target_dudes, att_arp_highest, melee_ranged) 
{

	// weapon_number: Weapon number
	// target_object: Target object
	// target_dudes: Target dudes
	// att_arp_highest: "att" or "arp" or "highest"
	// melee_ranged: melee or ranged
	...
	if (weapon_number>0) and (instance_exists(target_object)) and (owner=eFACTION.Imperium){
	    ...
	    shots_fired=wep_num[weapon_number];doom=0;
    
	    if (shots_fired!=1) and (obj_ncombat.enemy=5) and (melee_ranged!="melee") then doom=0.3;
	    if (shots_fired!=1) and (obj_ncombat.enemy=6) and (melee_ranged!="melee") then doom=0.4;
	    if (shots_fired!=1) and (obj_ncombat.enemy=7) and (melee_ranged!="melee") then doom=0.2;
	    if (shots_fired!=1) and (obj_ncombat.enemy=8) and (melee_ranged!="melee") then doom=0.4;
	    if (shots_fired!=1) and (obj_ncombat.enemy=9) and (melee_ranged!="melee") then doom=0.4;
	    if (obj_ncombat.enemy=11){att[weapon_number]=round(att[weapon_number]*1.15);apa[weapon_number]=round(apa[weapon_number]*1.15);}
	    if (obj_ncombat.enemy=10) and (obj_ncombat.threat=7) then doom=1;
    
	    damage_type="";
	    stop=0;targeh=target_dudes;
    
	    if (shots_fired=0) then exit;
	    if (ammo[weapon_number]=0){stop=1;exit;}
	    if (ammo[weapon_number]>0) then ammo[weapon_number]-=1;
    
    
    
    
	    if (att_arp_highest!="medi") then damage_type=att_arp_highest;
	    if (att_arp_highest="medi"){
	        damage_type="att";
	    ...
	    if (damage_type="status") and (stop=0) and (shots_fired>0){
	        ...
	        if (splash[weapon_number]=1) and (melee_ranged!="wall"){shots_fired=shots_fired*3;}
	        if (b>0) and (melee_ranged!="wall") and (instance_exists(target_object)){
	...
	if (instance_exists(target_object)) and (owner  = eFACTION.Player){
	    ...
	    if (weapon_number>0){shots_fired=wep_num[weapon_number];}
	    if (shots_fired=0) then exit;
	    /*if (weapon_number<-40){
	        if (weapon_number=-53){
	    ...
	    damage_type="";stop=0;targeh=target_dudes;
	    ...
	    if (att_arp_highest!="highest") then damage_type=att_arp_highest;
	    if (att_arp_highest="highest") and (weapon_number>0){
	        damage_type="att";if (att[weapon_number]>=100) and (apa[weapon_number]>0) then damage_type="arp";
	    }
	    if (att_arp_highest="highest") and (weapon_number=-51) then damage_type="att";
	    if (att_arp_highest="highest") and (weapon_number=-52) then damage_type="att";
	    if (att_arp_highest="highest") and (weapon_number=-53) then damage_type="att";   
	    ...
	    if (weapon_number>0) or (weapon_number<-40){// Normal shooting
	        ...
	        if (weapon_number>0){if (att[weapon_number]>0) and (stop=0) then that_works=true;}
	        ...
	        if (that_works=true){
	            ...
	            if (weapon_number<-40){
	                ...
	                if (weapon_number=-51){wii="Heavy Bolter Emplacement";at=160;ap=0;}
	                if (weapon_number=-52){wii="Missile Launcher Emplacement";at=200;ap=1;}
	                if (weapon_number=-53){wii="Missile Silo";at=250;ar=0;}
	            }
...
/*
 ...
 /*   
	    ...
	    if (weapon_number<-40){// Defenses shooting
	        ...
	        // }
	        /*if (damage_type="arp"){
	            a=ar*target_object.dudes_dr[targeh];
	            b=a-target_object.dudes_ac[targeh];
	            c=b*shots_fired;
  ...
  /*      
	    ...
	    if (weapon_number>0){// Normal shooting
	        if (att[weapon_number]>0) and (stop=0){
	            var a,b,c,eac,ap;ap=apa[weapon_number];
	            a=(att[weapon_number]/wep_num[weapon_number])*target_object.dudes_dr[targeh];// Average damage
	            ...
	            var casualties,ponies,onceh;onceh=0;ponies=0;
	            if (splash[weapon_number]=0) then casualties=min(floor(c/target_object.dudes_hp[targeh]),shots_fired);
	            if (splash[weapon_number]!=0) then casualties=floor(c/target_object.dudes_hp[targeh]);
	        ...
	        /*if (damage_type="arp") and (apa[weapon_number]>0) and (stop=0){
        
	        // if (damage_type="arp") and (stop=0){var a,b,c;
	            if (att[weapon_number]<=80) and (apa[weapon_number]=0) then apa[weapon_number]=att[weapon_number];
            
	            a=(apa[weapon_number]/wep_num[weapon_number])*target_object.dudes_dr[targeh];// Average damage
	            b=a-target_object.dudes_ac[targeh];// Average after armour
	            c=b*shots_fired;// New damage
	            ...
	            if (c>0) and (b>0){// Find number of casualties
	                ...
	                if (target_object.dudes_num[targeh]-casualties<0) and (onceh=0) then casualties=ponies;
	                if (casualties<0) then casualties=0;
	                scr_flavor(weapon_number,target_object,target_dudes,shots_fired-wep_rnum[weapon_number],casualties);
